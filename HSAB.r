...
data<-read.table('d:/cosas nuevas a guardar en H/Lonicera/Lonicera2012/paperherko/HSA/HSAB2.csv',sep=';',head=T)
...
###FLORAL TRAITS 
M01<-lm(seedsfloC~longcorC + tubewidthC+herkoRelaC+herkoRelaC2, na.action=na.omit, data=data)
summary(M01)
step(M01)

M02<-lm(seedsfloC~herkoRelaC+herkoRelaC2, na.action=na.omit, data=data)
summary(M02)
...
M011<-lm(fruitsetC~tubewidthC+tubewidthC2+herkoRelaC+herkoRelaC2, na.action=na.omit, data=data)
summary(M011)
step(M011)

M012<-lm(fruitsetC~herkoRelaC, na.action=na.omit, data=data)
summary(M012)

M013<-lm(fruitsetC~longcorC*herkoRelaC, na.action=na.omit, data=data)
summary(M013)
